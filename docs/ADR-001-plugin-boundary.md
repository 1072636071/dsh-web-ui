...
需求：在 dsh Web 输入框上方常驻「项目（工作区）选择器 + git 分支选择器 + Git 图谱」。git 能力必须在 host 进程执行（磁盘工作树真实 `git switch`），UI 在浏览器 React（`packages/client/*`）。需要先回答两个问题：外部插件能否向 Web 客户端注入模块、host 侧 RPC 是否可扩展。
...
- 槽位注册：`ctx.slots.register` 可注册进别家声明的槽位；对 `conversation.input.dock` 这类无 waitable 服务的槽位，用「检查 `slots.spec(name)` 否则 `slots.subscribe(name)` 延迟注册」范式（`ui-goal` 的 GoalDock 是现成模板，`ctx.inject(['slots','conversation',...])` 以 conversation 服务在作为注册安全信号）。
...
- 打开文件夹流程不做 draft 迁移（ui-conversation 的 selectWorkspace 才有 inputHub draft 搬运；外部包无访问面），切换即开新会话。
