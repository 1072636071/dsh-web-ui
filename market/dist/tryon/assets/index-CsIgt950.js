const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["./index-x6pE1ttQ.js","./git-DJDr4heb.js"])))=>i.map(i=>d[i]);
import{a as S,q as ie,S as re,r as oe,aa as ae,f as ce,_ as de,l as le,aj as ue}from"./index-x6pE1ttQ.js";import{SessionQueryEngine as pe,SESSION_QUERY_DEFAULT_PERSISTED_INSPECT_CONCURRENCY as z,SESSION_QUERY_READ_WINDOW_MAX as V,SessionQueryError as c,assertSessionHeadersCompatible as U,materializeSessionResultFilters as he,buildSessionEventSearchDocuments as _e,materializeSessionEventResultFilters as Ee,SessionSearchCursor as Se}from"./index-DI9z_KGV.js";import"./git-DJDr4heb.js";import"./index-DerFcqpW.js";import"./schemas-BjdgLEE2.js";import"./json-schema-processors-6dmQcDt8.js";const Je=8,fe=1146308689,ge=new Set(["search_state","persisted_sessions","persisted_docs","persisted_docs_data","persisted_docs_idx","persisted_docs_content","persisted_docs_docsize","persisted_docs_config"]);async function Ie(e){try{await(await ue(e,"wx",384)).close()}catch(t){if(t.code!=="EEXIST")throw t}}async function me(e,t){const s=e===":memory:"?e:oe(e);s!==":memory:"&&(await ae(ce(s),{recursive:!0,mode:448}),await Ie(s));const{DatabaseSync:n}=await de(async()=>{const{DatabaseSync:r}=await import("./index-x6pE1ttQ.js").then(o=>o.cK);return{DatabaseSync:r}},__vite__mapDeps([0,1]),import.meta.url),i=new n(s);try{const{application_id:r}=i.prepare("PRAGMA application_id").get(),{user_version:o}=i.prepare("PRAGMA user_version").get(),d=Ae(i);if(r!==0&&r!==1146308689)throw new Error(`session-search database at "${s}" belongs to another application`);if(r===0&&d.length>0)throw new Error(`session-search database at "${s}" is not an empty or recognized derived index`);return r===1146308689&&(Ne(s,d),o!==8&&Te(i,d)),i.exec(`PRAGMA journal_mode = ${t.toUpperCase()}`),ve(i),Re(i),i}catch(r){throw i.close(),r}}function Ae(e){return e.prepare("SELECT name FROM sqlite_master WHERE type = 'table' AND name NOT GLOB 'sqlite_*' ORDER BY name").all().map(t=>t.name)}function Ne(e,t){const s=t.filter(n=>!ge.has(n));if(s.length>0)throw new Error(`session-search database at "${e}" has unrecognized user tables: ${s.join(", ")}`)}function Te(e,t){for(const s of t)e.exec(`DROP TABLE IF EXISTS ${Le(s)}`);e.exec("PRAGMA user_version = 0")}function ve(e){e.exec(`PRAGMA application_id = ${fe}`),e.exec(`
