/**
 * Panel trust fence: loopback always passes; a live paired-device cookie
 * is an additional allow path when the remote-web-ui service is present.
 */
import type { IncomingMessage } from 'node:http'
import { describe, expect, it, vi } from 'vitest'
import { isPanelAllowed } from '../src/host/access.ts'
...
describe('isPanelAllowed', () => {
  it('allows loopback without a pairing service', () => {
    expect(isPanelAllowed({} as never, request())).toBe(true)
  })

  it('refuses a LAN client when no pairing service is present', () => {
    const ctx = { get: () => undefined }
    expect(isPanelAllowed(ctx as never, request({
  ...
  it('allows a LAN client with a live paired-device cookie', () => {
    ...
    expect(isPanelAllowed(ctx as never, req)).toBe(true)
