...
- **Master switch** (settings → Web UI Plugins → Right panel): `enabled`, on by default for standalone installs. Turn it off to unmount both panels, hide the floating expand button, and stop registering the `/aionui-panel/*` routes (workspace fs watching and git polling stop with them) — useful when another sidebar plugin owns the workspace chrome; the rest of the family bundle keeps working. Note: inside the `dsh-web-ui-all` aggregate bundle this switch defaults to **off** (the bundle's patch row sets `enabled: false`, so `dsh-better-sidebar` owns the right panel by default) — flip it on any time from the same settings card; running both panels at once is possible but they share the right edge, so prefer one at a time.
