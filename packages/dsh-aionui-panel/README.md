...
- **Right-panel provider choice** (settings → Web UI Plugins → Right panel): pick which right panel is active — `Use DSH-better-sidebar` (the **default**; aionui-panel is deprecated) or `Use aionui-panel (deprecated)`. Selecting aionui-panel mounts both panels, shows the floating expand button, and registers the `/aionui-panel/*` routes (workspace fs watching and git polling behind them); selecting DSH-better-sidebar keeps this panel unmounted and stops those routes and watchers. The choice is mutually exclusive: better-sidebar reads the same setting and does not mount while `Use aionui-panel` is selected (requires dsh-better-sidebar >= 0.13.0). The choice is the same everywhere (standalone installs and the `dsh-web-ui-all` aggregate bundle alike).
