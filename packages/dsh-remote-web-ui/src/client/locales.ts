...
export const zh = {
  ...
  'fence.unpaired.hint': '在桌面本机（http://127.0.0.1）打开配对面板生成配对链接，然后在此浏览器打开该链接即可完成配对。',
  ...
  'settings.requirePairingForLanHint': '开启：非本机回环的桌面 Web GUI 改走门控的 /remote/api，必须携带有效配对 Cookie；关闭：桌面继续走普通 /api（仅在该来源已被 SDK 信任时有意义），配对只管理令牌与状态。',
...
export const en = {
  ...
  'fence.unpaired.hint': 'Open the pairing panel on the desktop at http://127.0.0.1, then open the pairing link it mints in this browser.',
  ...
  'settings.requirePairingForLanHint': 'On: a desktop Web GUI at a non-loopback origin rides the gated /remote/api channel and must carry a live paired-device cookie. Off: the desktop stays on plain /api (only useful when that origin is already trusted for /api) and pairing only manages tokens/status.',
