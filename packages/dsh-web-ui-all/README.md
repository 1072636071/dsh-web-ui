...
- **Right panel transition**: `dsh-better-sidebar` owns the right-panel position by default (mutually exclusive with aionui-panel — better-sidebar reads the aionui provider choice and does not mount while `Use aionui-panel` is selected). The aionui-panel row stays installed but is **temporarily disabled** until its features are fully migrated to dsh-better-sidebar; its settings card (Settings → Web UI Plugins → Right panel) can switch the provider — `Use DSH-better-sidebar` (default) or `Use aionui-panel (deprecated)` — one dropdown, live switch, no reload, but switching back is not recommended before the migration completes.
