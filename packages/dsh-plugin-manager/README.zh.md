...
- 安装时冲突对账：官方模式对产品快照前后 diff；网关模式对每次 CLI 运行前后的 profile 层 diff，可撤销的动作给一键撤销。
...
- 安装时冲突检测报告安装实际改了什么（官方模式为产品行；网关模式为 profile 行与 bundle 条目）。两个用户插件之间的重复 insert id 认领在安装时无信号，在有失败环的运行时上于下次启动后呈现。
