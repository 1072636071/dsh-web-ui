...
# Or just this skin
dsh plugin --profile web add link:$(pwd)/packages/skins/xp
# Activate: dsh-skin use xp
```

`$(pwd)` is your clone of the dsh-web-ui monorepo. Once the skin packages are published to npm, install directly with `dsh plugin --profile web add @deepseek-ai/dsh-client-ui-skin-xp`.
