...
describe('Miku skin apply', () => {
  ...
  it('applies the Miku cursor to the body and restores it on dispose', async () => {
    ...
    expect(document.body.style.cursor).toContain('3 4')
    await fiber.dispose()
    expect(document.body.style.cursor).toBe('pointer')
  })

  it('restores the pre-existing cursor together with its priority', async () => {
  ...
  it('removes the cursor inline style on dispose when none existed before', async () => {
    fiber = await mount()
    await fiber.dispose()
    expect(document.body.style.getPropertyValue('cursor')).toBe('')
  })

  it('leaves the cursor untouched when dsh.miku.cursor is off, also after dispose', async () => {
