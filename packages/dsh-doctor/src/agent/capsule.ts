...
export interface CapsuleManifest { schemaVersion: 1; createdAt: string; dshExecutable: string; dshVersion: string; doctorPackage: string; doctorVersion?: string; rescueHome: string; status: 'staging' | 'verified' | 'failed' }
export interface CapsuleOptions { paths: DoctorPaths; dshExecutable: string; doctorSpec: string; doctorPackageDir?: string; doctorVersion?: string; now?: () => string; run?: typeof run }
...
export async function provisionCapsule(options: CapsuleOptions): Promise<CapsuleManifest> {
  ...
  const manifest: CapsuleManifest = { schemaVersion: 1, createdAt: (options.now ?? (() => new Date().toISOString()))(), dshExecutable: resolve(options.dshExecutable), dshVersion: version.stdout.trim(), doctorPackage: doctorSpec, ...(options.doctorVersion !== undefined ? { doctorVersion: options.doctorVersion } : {}), rescueHome, status: 'verified' }
