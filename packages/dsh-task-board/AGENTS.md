...
- 定时调度在浏览器端：`core/scheduler.ts` 每 60s 心跳（+ 标签页恢复即时补 tick），
  命中 cron 到期即触发，提前滚动到下一次匹配避免同 tick 双发；到期超过 2 个心跳
  周期的宽限窗视为错过——滚动到下一次匹配、不触发。
- 纯浏览器调度，**无服务端通道**：需 GUI 标签页打开才行，错过即跳过（不排队）；
  正在运行的任务到点被 runTask guard 拒绝，等下一次 cron。每次 tick 先经
  `refresh` 钩子重读持久化账本，已删除的任务不会从陈旧内存副本被触发。运行时人脸
  以结构接口注入，测试直接驱动 tick，无定时器。
