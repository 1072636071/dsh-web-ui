...
import { DECORATION_ASSET_PREFIX, petEntryView, petPackageRoot, codexPetsDir, type PetEntry, type PetRegistry } from './registry.ts'
 ...
 * pet.json, the entry's servable set (the sprite2d atlas, the animated-webp
 * state/transition webps, or the live2d model3.json plus its reference
 * closure — pet-center M3), and optional 'previews/<name>' media. The
 * servable match is an exact string comparison against scan-time normalized
 * paths, so crafted '..' or '.' segments never match; containedRealpath
 * stays as the second layer. Composed pets without a manifest file get a
 * synthesized pet.json.
 */
