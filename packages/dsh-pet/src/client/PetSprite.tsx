...
export function PetSprite(props: PetSpriteProps): ReactPortal {
  ...
  // Skipped on the animated-webp path (per-state webps load instead). A
  // custom visual (pet-center M3) replaces the atlas entirely.
  useEffect(() => {
    if (isWebp || props.visual !== undefined) return
    let cancelled = false
    const img = new Image()
    img.onload = () => {
  ...
  }, [definition.atlasUrl, isWebp, props.visual])
  ...
  useEffect(() => {
    ...
    raf = requestAnimationFrame(tick)
    return () => cancelAnimationFrame(raf)
  }, [animation, phase, cell, columns, rows, tracks, sequences, isWebp, props.visual])
