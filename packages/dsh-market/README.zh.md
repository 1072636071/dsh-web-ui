...
DSH Web GUI 设置页的市场卡片：在 GUI 内浏览 [dsh-market.com](https://dsh-market.com) 的皮肤、宠物与社区插件，并一键安装到本机。

## 功能

- 三类目录（皮肤 / 宠物 / 插件），排序与市场站一致：设备点赞优先（同票回落清单顺序）、搜索框、每卡预览链接（皮肤打开实时试穿模拟器）。
...
重启 `dsh web` 后，设置页出现「市场」分区（与皮肤中心、宠物、社区插件同级）。
...
- 资产安装要求市场站可达；清单或下载失败时，已存在的资产目录保持原样。
...
- 市场站每项资产带明确的文件清单，`scripts/market-build` 重新生成 `market/dist` 后，新皮肤包即自动可装。
