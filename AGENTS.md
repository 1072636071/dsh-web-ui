...
Issue 与 PRD 以本地 markdown 存放于仓库 `.scratch/` 下（`<NN>-<feature>/` 目录 +
`Status:` 行）。参见 `docs/agents/issue-tracker-local.md`。

### triage 标签

五个标准 triage 角色标签：`needs-triage` / `needs-info` / `ready-for-agent` /
...
所有临时脚本统一放在仓库 `.temp/scripts/` 下；其他临时文件（脚本输出、日志等）
也要分类，放在 `.temp/` 的子目录下（如 `.temp/output/`、`.temp/logs/`），保证仓库
根目录干净。
