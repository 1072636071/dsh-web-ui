...
`packages/` contains feature plugins, the Skin Center, and the `dsh-web-ui-all` aggregate package. The `dsh-market` package is the DSH Market hub: one settings entry (`settings.section` id `dsh-market`) declaring the `dsh-market.tab` child slot, with the Skin Center / Pet / Community Plugins / Store cards registering as tabs (each falls back to its own first-level section when the hub is not installed; the seat machine lives in `shared/client/settings/market-tab.ts`). The skin-center npm package ships only the `skins/blue-fantasy` asset (files whitelist); every other skin stays in `packages/skins/skin-center/skins/` as the market-build / gallery source and installs on demand into `$DSH_HOME/skins/<id>/` from the market.
