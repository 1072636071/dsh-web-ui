...
`packages/` contains feature plugins, the Skin Center, and the `dsh-web-ui-all` aggregate package. The `dsh-market` package is the DSH Market store: one settings entry (`settings.section` id `dsh-market`) rendering the store card (browsing dsh-market.com manifests for skins / pets / plugins with one-click install into the DSH home directories). The Skin Center and the Pet plugin register their own first-level settings sections listing only installed items; the community-plugins package is the community.json data source (the market plugin manifest and the store plugin list derive from it) and keeps an inert cordis row so existing profiles keep resolving. The skin-center npm package ships only the `skins/blue-fantasy` asset (files whitelist); every other skin stays in `packages/skins/skin-center/skins/` as the market-build / gallery source and installs on demand into `$DSH_HOME/skins/<id>/` from the market.
