...
description: Safely upgrade dsh-web-ui to a compatible, official @deepseek-ai SDK release from npm using an isolated worktree, explicit compatibility review, full CI-equivalent validation, and controlled DSH Web GUI rollout. Use for DSH SDK dependency upgrades, official SDK version checks, or SDK compatibility migrations in this repository.
whenToUse: The user asks to upgrade, update, migrate, or check the official DeepSeek Harness SDK packages used by dsh-web-ui, including @deepseek-ai/dsh-*, @deepseek-ai/cordis, @deepseek-ai/cosmokit, or @deepseek-ai/schemastery. Do not use for upgrading the DSH application/source checkout itself, releasing @linxin666 packages, or ordinary plugin feature work.
---

# dsh-web-ui Official SDK Upgrade

Use this Skill only for the dsh-web-ui monorepo. It upgrades the repository's official npm SDK dependencies; it never modifies a DeepSeek Harness source checkout. For upgrading the DSH application itself, use dsh-upgrade instead. For publishing this monorepo, use dsh-web-ui-release instead.
...
6. If the new SDK changes types, APIs, imports, Cordis composition behavior, client-platform boundaries, or generated bundle behavior, make the smallest compatible code and test changes. Follow the repository's host/client separation and do not add source-checkout dependencies as a shortcut.

## 4. Validate at CI Strength

Run the CI-equivalent gate sequence in the isolated worktree, recording the actual command and result for every gate:
...
Local compilation is necessary but not sufficient. After the isolated worktree passes the static gates:
