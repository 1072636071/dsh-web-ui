...
const SOURCE_DIR = join(REPO_ROOT, 'shared', 'client', 'settings')
const CONSUMERS = ['dsh-pet', 'dsh-task-board', 'dsh-remote-web-ui', 'dsh-live-stats', 'dsh-tool-describe-image']
const FILES = ['settings-form.ts', 'PluginSettingsCard.tsx', 'settings-card.module.css']
...
export function copyEntries(root = REPO_ROOT) {
  ...
  for (const pkg of CONSUMERS) {
    for (const file of FILES) {
      entries.push({
        source: join(root, 'shared', 'client', 'settings', file),
        target: join(root, 'packages', pkg, 'src', 'client', file),
        file,
      })
    }
  }
...
export function headerFor(file) {
  const body = `Generated by scripts/sync-shared.mjs from shared/client/settings/${file}. Do not edit this copy; edit the shared source and run "node scripts/sync-shared.mjs".`
  return file.endsWith('.css') ? `/* ${body} */` : `// ${body}`
}

/** Render a consumer copy from its shared source. */
export function renderCopy(sourceText, file) {
  return `${headerFor(file)}
${sourceText}`
}

/** Recover the shared source text from a rendered copy (undefined when the header is missing or mangled). */
export function stripHeader(rendered, file) {
...
export async function applySync(root = REPO_ROOT) {
  let written = 0
  for (const { source, target, file } of copyEntries(root)) {
    const sourceText = await readFile(source, 'utf8')
    const rendered = renderCopy(sourceText, file)
...
export async function checkSync(root = REPO_ROOT) {
  ...
  for (const { source, target, file } of copyEntries(root)) {
    ...
    const stripped = stripHeader(rendered, file)
