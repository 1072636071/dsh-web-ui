 ...
 */
export function extractCssPayloads(bundleText, skinDir) {
  const payloads = []
  let searchFrom = 0
  for (;;) {
 ...
 */
export function detectJsEffectTodos(bundleText, { backdropMigrated, faviconFound }) {
  const todos = []
 ...
 */
export function rewriteSelectorText(selector, bodyAttr, stats) {
  let s = selector.trim()
 ...
 */
export function dropDeadPrivateVars(files) {
  const removed = []
 ...
 */
export function draftHooks({ skinId, name, favicon, backdropMigrated, todos }) {
  const lines = []
  lines.push('/**')
